...
public class GuildManager {
  ...
  public GuildManager() throws SQLException {
    ...
    for (DbRow row : DB.getResults("SELECT * FROM guilds")) {
      ...
      List<Member> members =
          DatabaseUtils.serializeList(row.getString("Members")).stream()
      ...
      this.guildMap.put(name, new Guild(name, members, nexusLocation, allies, home, chunks, storage,
  ...
  public void save() {
    this.guildMap.values().forEach(guild -> {
        try {
            DB.executeUpdate(
                "INSERT OR REPLACE INTO guilds(Name, Members, Allies, Home, Nexus,"
