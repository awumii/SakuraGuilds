...
public class AllyAcceptCommand implements SubCommand {
    ...
    public void handle(GuildManager manager, Player player, String[] args) {
        ...
        if (args[1].equals("IJAD98jdksldM")) {
            ...
            ChatUtils.guildAlert(guild, guild.member(player).displayName() + " &7odrzucił zaproszenie sojuszu od &6" + other.name());
            ChatUtils.guildAlert(other, guild.member(player).displayName() + " &7z gildii &6" + guild.name() + " &7odrzucił wasze zaproszenie do sojuszu.");
        }
    }
}
